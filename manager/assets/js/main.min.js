        ...
        success: function(data)
        {
            ...
            else
            {
                queueData.forEach(function(queue) {
                    node = queueTable.row.add( [ queue.id, queue.process_type.type, queue.reference_id ] ).draw().node();
                });
                M.toast({html: '¡Cola cargada!'});
            }
        },
        error: function(err)
        {
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            logs.forEach(function(log) {
                logsTableSel.row.add( [ log ] ).draw().node();
            });
        ...
        {
            console.error(err);
            M.toast({html: 'No existen logs para la secsión indicada.'});
        },
        complete: function(){
...
function loadLogsBot()
{
    $.ajax({
        ...
        success: function(data)
        {
            var logs = data.data;
            logs.forEach(function(log) {
                var logTime = new Date(log.datetime),
                    node = logsTableBot.row.add( [ log.id,
                        timeago().format(logTime, 'es')
                        ,log.message ] ).draw().node();
            });
            M.toast({html: '¡Logs cargados!'});
        },
        error: function(err)
...
function statusBot() {
    $.ajax({
        ...
        error: function (err) {
            setStatus("Error al cargar estado", "Error de procesamiento con REST.", icons["_errorLoading"]);
            $("#statusChecker").attr("disabled", false);
            console.error(err);
        }
    });
